...
- **Candidates API**: `POST /api/candidates`, `GET /api/candidates`, `GET /api/candidates/:id`, `PATCH /api/candidates/:id`, `POST /api/candidates/account-setup` (email-check → `{ status: "created"|"existing"|"updated", candidateId, candidate }`), `POST /api/candidates/login` (test-mode login by email → `{ exists, candidateId, candidate }`), `POST /api/candidates/check-email` (→ `{ exists, candidateId }`)
- **Account Access Step (Step 2)**: Replaced single sign-up form with a branching UI: (1) **Choice screen** — two cards: "I already have an account" (Login) / "I'm new here" (Sign up); (2) **Login form** — email + password, POSTs to `/api/candidates/login`; if email not found shows inline error + "Switch to sign up" link; on success advances to Account Success with "Signed In" badge; (3) **Sign-up form** — email + password + confirm, POSTs to `/api/candidates/account-setup`; if duplicate email found shows warning card + "Log in instead" CTA; both forms have "Back to options" link; (4) footer button shows "Sign In" for login mode, "Create Account" for signup mode, hidden for choice/duplicate screens
- **accountMode state**: `"choice" | "login" | "signup" | null` — controls which sub-view AccountAccessStep renders; reset on handleRetake and on step 1→2 transition
- **Duplicate prevention**: `getCandidateByEmail` in DbStorage; login checks email exists (test-mode); signup checks for duplicate email before creating — both guard against duplicate candidate/account records
- **Culture Evaluation Persistence**: Separate `candidate_culture_evaluations` table (id, candidateId FK→candidates, answers jsonb, valueScores jsonb, overallScore, alignmentLevel, summary, traits[], completedAt, createdAt, updatedAt); `POST /api/candidates/:id/culture-evaluation` upserts the evaluation + updates `candidates.cultureScore`; `GET /api/candidates/:id/culture-evaluation` fetches it; culture evaluation saved before showing Culture Result step; "Evaluation Saved" badge shown on success, save-error notice shown on failure; evaluations always linked to candidateId (never orphaned)
