...
- **Talent Pool**: Allows Talent Acquisition users and Clients to discover pre-assessed candidates. Features client-side match scoring, a `MATCH_THRESHOLD` of 30, match labels (Best, Strong, Possible), filters (category, experience, location, work setup), a shortlist function. Each card shows the talent's real saved name (via `getTalentDisplayName`), profile photo (via `candidatePhotoSrc`), and a "Full Profile" button linking to `/talent-profile/:id`. `Candidate XXXXXX` only appears as fallback when no name is stored. Contact details (email/phone) remain admin/TA-only. The talent list query uses `staleTime: 0` + `refetchOnWindowFocus: true` to always reflect profile edits. `passwordHash` is stripped from all candidate API responses.
- **Talent Profile Page** (`TalentProfile.tsx`): LinkedIn-inspired full profile page at `/talent-profile/:id`. Features: (1) A sticky profile-specific navbar (`ProfileNavbar`) with OnSpot branding, navigation links (Home, Talent Pool, Find Work, Insights), and auth controls (Sign in / Sign out depending on owner state); (2) A sticky horizontal section-tab bar (`SectionTabs`) with smooth-scroll navigation to: Overview, About, Skills, Experience, Education, Certifications, Portfolio, Resume, Preferences, Contact — uses IntersectionObserver to highlight the active tab while scrolling; (3) Purple/indigo gradient cover banner with overlapping profile avatar and photo upload (owner only); (4) Two-column layout (main content + sidebar) that collapses to single-column on mobile; (5) Profile strength tracker sidebar. Header shows the talent name prominently below the avatar: `displayName` → `fullName` → `Candidate XXXXXX` fallback. Name is inline-editable by the owner (max 80 chars). Inline editable sections (owner only): About/summary, headline, preferences, portfolio links. Dedicated cards for Skills, Experience, Education, Certifications, Values Alignment (culture score), Portfolio & Links, Resume, and Contact (role-gated for admin/TA only). All edits save via `PATCH /api/candidates/:id`. Education and certifications are stored as `jsonb` arrays. The `Section` component accepts an optional `id` prop with `scroll-mt-28` for correct offset under the double-sticky header.
- **Talent Profile Ownership Auth**: Candidates authenticate with email + password to edit their own profile. Auth endpoints: `POST /api/talent-auth/login` (email+password → 30-day JWT with `type: "candidate"`), `POST /api/talent-auth/set-password` (first-time password setup, requires email + candidateId), `GET /api/talent-auth/me` (verify session). Token stored as `talent_profile_token` in `localStorage`. `PATCH /api/candidates/:id`, `POST /api/candidates/:id/photo`, and `POST /api/candidates/:id/resume` all require either a talent JWT (owner) or admin JWT. The `candidates` table has a `password_hash` column (bcrypt, nullable). Edit controls and photo upload overlay are hidden until authenticated as owner. "Edit my profile" button is completely hidden from non-owners; a discreet "Own this profile? Sign in to manage it" text link appears at the bottom footer of the profile for unauthenticated non-admin visitors. Shared `TalentLoginModal` component (`client/src/components/TalentLoginModal.tsx`) exports modal + auth utils (`TOKEN_KEY`, `TalentAuthState`, `loadTalentAuth`, `saveTalentAuth`, `clearTalentAuth`). When no `profileId` is provided to `TalentLoginModal`, the set-password flow is hidden (sign-in only mode).
- **TalentAccountPrompt**: A floating popup on the Talent Pool page that appears after 5 minutes for non-authenticated visitors. Uses `sessionStorage` (`talentAccountPromptDismissed`) to remember dismissal per session. Skips if the user is already authenticated as talent (`talent_profile_token` in localStorage). Has minimize/floating-pill toggle behavior (similar to WeeklyNotes on Insights page). "Create Talent Account" navigates to `/find-best-matches`; "Sign in" opens the shared `TalentLoginModal` and on success navigates to `/talent-profile/:candidateId`. Rendered via `createPortal` into `document.body`. Component lives in `TalentPool.tsx`.
- **Candidate Schema Extensions**: `candidates` table now includes `displayName` (custom public-facing name), `profilePhotoUrl`, `resumeUrl`, `resumeFileName`, `headline`, `linkedinUrl`, `githubUrl`, `portfolioUrl`, `websiteUrl`, `availability`, `education` (jsonb), `certifications` (jsonb).
