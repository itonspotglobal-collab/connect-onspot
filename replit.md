...
- **Candidates API**: `POST /api/candidates`, `GET /api/candidates`, `GET /api/candidates/:id`, `PATCH /api/candidates/:id`, `POST /api/candidates/account-setup` (email-check → `{ status: "created"|"existing"|"updated", candidateId, candidate }`)
- **Duplicate prevention**: `getCandidateByEmail` in DbStorage; account-setup route checks email before creating — returns `status: "existing"` if found; UI branches to "Existing Account Found" card with "Continue with Existing Profile" / "Update Profile" CTAs; "Use a different email" resets the form
