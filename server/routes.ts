...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // OpenAI Chat API endpoint for Vanessa AI Assistant
  app.post("/api/chat", async (req: Request, res: Response) => {
    try {
      ...
      // Assistant ID for Vanessa AI
      const assistantId = "asst_l95RxbCxER9jjTZliJQzEpt2";
      ...
      if (!textContent || textContent.type !== "text") {
        console.error("❌ No text content in assistant response");
        return res.status(500).json({
