...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // GET /api/admin/jobs - List all jobs for admin portal
  app.get("/api/admin/jobs", async (req: Request, res: Response) => {
    try {
      const allJobs = await storage.listAllJobs();
      res.json(allJobs);
    } catch (error: any) {
      console.error("Error listing admin jobs:", error.message);
  ...
  app.post("/api/admin/jobs", async (req: Request, res: Response) => {
    try {
      const { title, description, company, location, category, contractType, hourlyRateMin, hourlyRateMax, experienceLevel } = req.body;
      
      if (!title || !description) {
        return res.status(400).json({ error: "Title and description are required" });
      }

      // Find an admin user to associate jobs with
      ...
      console.error("Error creating admin job:", error.message);
      res.status(500).json({ error: "Failed to create job" });
    }
  });

  // PATCH /api/admin/jobs/:id/toggle-status - Toggle job open/closed
  app.patch("/api/admin/jobs/:id/toggle-status", async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      const updatedJob = await storage.updateJob(id, { status: newStatus } as any);
      res.json(updatedJob);
    } catch (error: any) {
  ...
  app.delete("/api/admin/jobs/:id", async (req: Request, res: Response) => {
    try {
      ...
      await storage.updateJob(id, { status: "cancelled" } as any);
