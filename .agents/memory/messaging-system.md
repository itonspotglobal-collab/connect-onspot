...
- **Cross-message privacy:** inspect at most five consecutive same-sender messages in one thread over three minutes; another participant's reply ends the deterministic sequence.
- **Atomic retroactive masking:** serialize sends per thread and commit prior-message masking/flagging with the sanitized new message so no committed state exposes a reconstructable split value.
- **Minimal span retention:** map deterministic and validated Vanessa ranges back to exact message character spans. Flag every involved message, but preserve unrelated text; reject broad Vanessa whole-context ranges.
...
**Why:** Message rows and thread participants use users.id, so allowing policy acceptance under a candidate id creates a split identity that can bypass one gate and fail another. Server-side versioning prevents stale clients or direct API calls from skipping renewed consent. Without a thread lock and one transaction, concurrent sends can select the wrong context or briefly retain a reconstructable split secret; blanket message replacement unnecessarily destroys legitimate conversation history.

**How to apply:** Any new messaging write path must resolve the canonical users.id, verify the current policy version for user-authored content, and expose only safe redaction booleans/category names to clients. Context-aware paths must keep the same sender/thread/window bounds, transaction ordering, range mapping, and notification-after-commit rule.
