...
- `talent_signed_at` — admin-recorded; informational; does NOT trigger hired on its own.
- `onspot_signed_at` — the execution trigger: when set, contract → `signed`, submission → `hired`.
...
```
offer_accepted → [POST contract] → contract_sent → [PATCH sign talent] → contract_sent
  → [PATCH sign onspot] → signed → submission: hired
...
| PATCH sign | status = 'voided' | 409 contract_voided |
| PATCH sign | status = 'signed' | 409 contract_already_signed |
...
Talent routes use standard `authenticateJWT` + `talent_id` ownership check.
...
| `PATCH /api/admin/hiring-contracts/:id/sign` | Admin JWT | body: `{ signerType: 'talent'\|'onspot', signedAt? }` |
