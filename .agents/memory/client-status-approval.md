...
**How to apply:** Keep Client request creation, cancellation, and request notifications separate from `job_submissions.status`. At approval, lock the request and application, reject stale requests before email, then update the request, canonical status, history, Talent notification, and Client approval alert in the same post-delivery transaction. If delivery succeeds but that transaction fails, retain sent-email evidence so an Admin-only finalize action can safely commit the pending request without sending again.