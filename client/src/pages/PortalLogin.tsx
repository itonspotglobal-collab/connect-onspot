...
export default function PortalLogin() {
  ...
  const portalParam = params.get("portal") as PortalType | null;
  const emailParam = params.get("email") || "";

  const [step, setStep] = useState<PageStep>("login");
  const [selectedPortal, setSelectedPortal] = useState<PortalType | null>(
    portalParam === "client" || portalParam === "talent" ? portalParam : null,
  );
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className="bg-white/10 border-white/20 text-white placeholder:text-white/40 focus:border-[#3A3AF8] h-12 pr-10"
                />
                <button type="button" onClick={() => setShowForgotPw(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 hover:text-white/90 transition-colors">
          ...
          <p className="text-white/40 text-xs">
            ...
            <Link href="/">
              <span
                ...
                onClick={() => {
                  // Store flag so homepage opens Access Portal on mount
                  sessionStorage.setItem("openAccessPortal", "signup");
                }}
              >
                Create Account
              </span>
